# 2024-04-08 total solar eclipse — the shoot, and what the data needed

> **This is a record, not instructions.** It describes the original PixInsight
> and Node implementation and the reasoning behind every decision the current
> pipeline inherited — why the Moon needs its own reference frame, why totality
> is bracketed by blown frames rather than a threshold, why the sunspot search
> normalises radially, what the field rotation is bounded at. All of that still
> governs the code.
>
> The commands and file paths in here do not. That implementation is gone from
> `main`; it is preserved on the `pixinsight-legacy` branch. To actually run
> something, read [`../README.md`](../README.md).
...
The original implementation used `pix-planetary` as a library, by absolute path —
`ser-stack.js`, its `lib/fftalign.jsh`, `xisf-preview.js` and `pi-lock.mjs` were
called where they sat in `D:/projects/pix-planetary`, never copied. Needing a
second private repository at a fixed path is one of the reasons that version was
replaced.
...
*(Superseded. Kept because the ordering constraints below are real and the
current pipeline enforces the same ones.)*
