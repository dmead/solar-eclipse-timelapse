...
looked like in person.
    ...
    python -m ecl.gen_timelapse --out out

Writes `configs/timelapse.json`, which `ecl.tl_render` consumes.
"""
...
# Levels in segments.json are fractions of full scale, so full scale is 1.0.
# It was 65535 while the stats were raw 16-bit counts; both changed together
# and every gain here is a ratio of the two, so the numbers are unchanged.
FULL_SCALE = 1.0
...
def level(point):
    ...
    return max(point["med"] - point["black"], 1.0 / 65535.0)
        ...
        for k, s in enumerate(run):
            ...
            if s["kind"] == "stable":
                ...
                for p in pts:
                    ...
                    cap = (TRANSITION_CEILING * TARGET[s["state"]] * FULL_SCALE
