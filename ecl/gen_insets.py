...
by fitting the terminator the Moon casts on the disc, which needs no ephemeris.
    ...
    python -m ecl.gen_insets --out out --data /path/to/captures

Run after `ecl.smooth_track`; rewrites configs/timelapse.json with an "insets" list
on every frame.
"""
...
# by the zoom, so a bigger zoom means a tighter box.
#
# PANEL_PX is a DEFAULT, not a constant: tune() sets it to panels.size_frac of
# the output's short side, which is what that documented setting always
# claimed to do and never did. Held at 420 it was a quarter of this camera's
# 1680 px output and wider than the whole frame on a smaller one - the
# renderer died trying to broadcast a 420 px panel into a 320 px canvas.
PANEL_PX = 420
ZOOM = 3.0

# Drizzle factor of the render this plans for; FINE px = DRIZZLE x plane px.
DRIZZLE = 2
...
def tune(out_dir, log=None):
    ...
    global MAX_PANELS
    global CONTINUITY_MAX_EXTRA, SPREAD_WEIGHT, DRIZZLE
...
def panel_scale(panel, other):
    """Convert between a panel's magnification and the plane half-width it shows.

    The relation is its own inverse: half = panel / (2 * DRIZZLE * zoom), and
    zoom = panel / (2 * DRIZZLE * half). Both directions were written out as
    panel / (4.0 * x) in six places, which is this formula with DRIZZLE fixed
    at 2 - correct for the camera it was written on, and silently wrong on any
    run the survey drops to drizzle 1.
    """
    return panel / (2.0 * DRIZZLE * other)
...
def green_plane(src, index):
    """Green at PLANE resolution, in counts.

    Which resolution that is belongs to the source and not to this module: a CFA
    sensor gives half-size superpixel planes, an already-demosaiced frame gives
    full size. This used to reshape the raw buffer as uint16 and pick the Bayer
    green itself, which silently assumed both a mosaic and 16 bits and was the
    reason the panel stage could not read a folder of camera images.

    Counts, not [0, 1], because every threshold below this line was measured in
    counts. `beadwindow` scales the same way for the same reason.
    """
    return src.green(index) * 65535.0


def halpha_plane(src, index):
...
def find_sunspot(frame, cx, cy, r_sun):
...
def best_sunspot(partials, r_sun, n_try):
...
def find_prominences(frame, mx, my, r_moon, n):
    ...
    """
    with open_source(frame["src"]) as src:
        g = halpha_plane(src, frame["index"])
    h, w = g.shape
    yy, xx = np.mgrid[0:h, 0:w]
...
def size_to_arc(g, band, picks, mx, my, r_moon, med, sigma):
    ...
    cap = panel_scale(PANEL_PX, ARC_MIN_ZOOM)   # plane half-width at min zoom
...
# Baily's beads.
from .beadwindow import MIN_AREA as BEAD_MIN_AREA  # noqa: E402
...
def fit_moon_track(partials, r_sun, r_moon, n_try):
    ...
    ts, ox, oy = [], [], []
    for f in partials[::step]:
        r = fit_moon(f, f["cx"], f["cy"], r_sun, r_moon)
...
# Gap between a panel and the frame edge, in fine px. This must match M in
# tl_render.draw_insets: the planner decides which slot clears the disc and
# the renderer decides where the slot is, so a disagreement puts panels
# somewhere the planner never checked.
PANEL_MARGIN = 24
...
def main():
    ...
    ap.add_argument("--panel", type=int, default=0,
                    help="panel edge in rendered px; 0 = panels.size_frac "
                         "of the output short side")
    args = ap.parse_args()
    P = tune(args.out, log=print)

    cfgpath = os.path.join(args.out, "configs", "timelapse.json")
    cfg = json.load(open(cfgpath))
    # Every frame read below goes through `src`, so --data redirects them all,
    # exactly as it does for centres, track and render. It used to join --data
    # to each frame's bare filename instead, which only worked when a capture
    # was a single file sitting directly in the data directory.
    serio.restage(cfg, args.data)

    """
    Panel size follows the output frame, because a panel is a fraction of what
    the viewer sees and not a number of sensor pixels.

    `panels.size_frac` has been in the config, and documented as "panel edge as
    a fraction of the output short side", since the config existed. It was never
    read. The panel was 420 px on every run - a quarter of this camera's 1680 px
    short side, which is where the number came from, and 131% of the short side
    of a 320 px output, where the renderer raised
    `could not broadcast (420,420) into (296,20)` from inside a worker.
    """
    if not args.panel:
        global PANEL_PX
        short = DRIZZLE * min(cfg["outW"], cfg["outH"])
        args.panel = max(32, int(round(P.get("panels.size_frac", 0.25) * short)))
        PANEL_PX = args.panel
        print("  panel %d px = %.2f of the %d px output short side"
              % (args.panel, P.get("panels.size_frac", 0.25), short))
    ...
    mvx, mvy, nfit, resid = fit_moon_track(partials0, r_sun, r_moon,
                                           MOON_TRY_FRAMES)
    ...
    ox, oy, depth, seed = best_sunspot(partials, r_sun, SPOT_TRY_FRAMES)
    ...
    for fn in sorted(by_file, key=lambda f: levels[by_file[f][0]][0]["utc"]):
        ...
        mid = fs[len(fs)//2]
        mx, my = moon_of(mid)
        pang[fn] = find_prominences(mid, mx, my, r_moon,
                                    MAX_PANELS)
    ...
    # Panel rectangles in PLANE px, matching draw_insets, which works in fine
    # px with the same margin - hence dividing both by DRIZZLE.
    _p, _m = args.panel / DRIZZLE, PANEL_MARGIN / DRIZZLE
    ...
    # Never leave a feature without a slot: a cramped panel beats no panel. The
    # floor is the panel CAP and not a hardcoded four - it was four while
    # MAX_PANELS was six, so a tight frame carrying five or six features asked
    # for a permutation longer than the slot list and the assignment indexed off
    # the end of it. Both numbers are configurable; neither may be assumed.
    _floor = min(8, max(1, MAX_PANELS))
    if len(slots) < _floor:
        slots = sorted(range(8), key=lambda i: -slot_clear[i])[:_floor]
        print("  frame is too tight for a %.0f px gap; using the %d roomiest "
              "slots" % (_need, _floor))
    ...
    def plane_of(f):
        if src_cache[0] != f["src"]:
            if src_cache[1] is not None:
                src_cache[1].close()
            src_cache[1] = open_source(f["src"])
            src_cache[0] = f["src"]
        return green_plane(src_cache[1], f["index"])
            ...
            if any(k in near for k in run):
                ...
                print("  beads: %s f%d-%d, %d frames, deg %d, box half %.0f px "
                      ...
                      % (frames[run[0]]["file"], rows[0][0], rows[-1][0],
                         len(run), deg_fit, half, panel_scale(args.panel, half),
                         dev[len(dev)//2]))
                for k in run:
                    t = frames[k]["index"]
    ...
    for f in frames:
        ...
        if f["state"] == "unfiltered":
            ...
            if bead:
                bx, by, bhalf = bead
                feats.append((bx, by, "baily's beads",
                              panel_scale(args.panel, bhalf)))
            ...
            if not f.get("resolve"):
                for pr in pang.get(f["file"], []):
                    ...
                    z = args.zoom if half is None else panel_scale(args.panel, half)
            ...
            if ths:
                ...
                czoom = panel_scale(args.panel, half)
        ...
        # Capped by the panel limit AND by the slots that exist. The layout
        # search below builds an injective feature->slot assignment, so more
        # features than slots has no answer at all.
        feats = feats[:min(MAX_PANELS, len(slots))]
