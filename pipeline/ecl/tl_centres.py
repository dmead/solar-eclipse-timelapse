    ...
    try:
        for k, fr in enumerate(frames):
            if fr["src"] != cur_path:
                ...
                w, h = ser.raw_width >> 1, ser.raw_height >> 1
                cur_path = fr["src"]
            ...
            if fr["state"] == "unfiltered":
                if r_moon <= 0:
                    r_moon = int(round(r_sun / SUN_OVER_MOON)) if r_sun > 0 else 288
                    log(f"  moon limb radius {r_moon} px (half-res)")
                grad = log_gradient(g)
                ...
                if r_sun <= 0:
                    ...
                    wide = kasa_fit(pts, bcx, bcy, max(bbox_r, 60),
                                    60, min(w, h) / 2)
                    if wide["arc"] < 120:
                        grad = log_gradient(g)
                        best = None
                    ...
                    log(f"  sun limb radius measured at {r_sun:.1f} px (half-res)"
                        f" = {r_sun * 2:.0f} px full  [arc {wide['arc']} deg,"
                        f" rms {wide['rms']:.2f}]")
